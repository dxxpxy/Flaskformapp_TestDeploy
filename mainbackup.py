...
def addinvoice():
    ...
    if request.method == 'POST':
        ...
        #     flash('Enter customer name', category='redlight')
        
        # elif len(customeraddress) < 3:
        #     flash('Address can\'t be less then 3 characters', category='redlight')
            
        # elif len(invoiceno) < 1:
        #     flash('Enter invoice number', category='redlight')
            
        # elif len(description) < 1:
        #     flash('Please add a description', category='redlight')
            
        # elif len(invoicetotal) < 1:
        #     flash('Enter invoice total.', category='redlight')
            
        # elif date is None or date == '':
        #     flash('Please add a date.', category='redlight')    
...
def viewinvoice():
    ...
    results = db_func.run_query('SELECT * FROM invoice')
    return render_template("invoice.html", results=results)        # Pass data to HTML template for display
